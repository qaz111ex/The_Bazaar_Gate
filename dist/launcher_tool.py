"""
Bazaar Mod Launcher - 游戏模组启动器

该模块提供了一个图形界面工具，用于管理The Bazaar游戏的模组加载流程。
主要功能包括：模组备份/恢复、启动器参数捕获、游戏启动等。

Author: BazaarModLauncher Team
Version: 1.1.0
"""
...
class AppConfig:
    ...
    DEFAULT_MOD_ITEMS: Tuple[str, ...] = ('BepInEx', 'BazaarPlusPlus', 'doorstop_config.ini', 'winhttp.dll')
...
class BazaarGate:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.settings_file: str = "settings.txt"
        self.backup_folder: str = "mod_backup"
        self.log_file: str = "launcher.log"
    ...
    def _setup_styles(self) -> None:
        ...
        style.configure('Primary.TButton', font=('Microsoft YaHei UI', 10, 'bold'), foreground='#ffffff', background='#0078d4')
    ...
    def _create_main_page(self) -> None:
        ...
        self.language_label = ttk.Label(lang_frame, text=self.lang.t('language_setting_label', '🌐 语言'), style='Header.TLabel')
        ...
        self.log_text = tk.Text(log_card, height=AppConfig.LOG_TEXT_HEIGHT, wrap=tk.WORD, font=('Consolas', 9),
    ...
    def _create_settings_page(self) -> None:
        ...
        self.mod_files_text = tk.Text(text_frame, height=AppConfig.MOD_FILES_TEXT_HEIGHT, wrap=tk.WORD, font=('Microsoft YaHei UI', 10),
        ...
        self.save_settings_btn = ttk.Button(button_frame, text=self.lang.t('save_settings_btn', '💾 保存配置'), command=self._save_mod_settings, style='Primary.TButton')
        self.save_settings_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=6, padx=(0, 4))

        self.reset_default_btn = ttk.Button(button_frame, text=self.lang.t('reset_default_btn', '🔄 恢复默认'), command=self._reset_to_default, style='Secondary.TButton')
        self.reset_default_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=6, padx=(4, 0))
    ...
    def _apply_language_to_settings_page(self) -> None:
        ...
        self.reset_default_btn.config(text=self.lang.t('reset_default_btn', '🔄 恢复默认'))
    ...
    def log_t(self, key: str, *args: Any, level: str = "INFO", default: Optional[str] = None) -> None:
        """
        记录翻译后的日志消息。
    ...
    def _update_status_label(self, message: str, level: str) -> None:
        ...
        colors = {"ERROR": "red", "SUCCESS": "green", "WARNING": "orange", "INFO": "blue"}
    ...
    def _check_default_paths(self) -> None:
        ...
        检查并设置默认路径。
        """
        if self.default_game_path and os.path.exists(os.path.join(self.default_game_path, AppConfig.GAME_EXE_NAME)):
    ...
    def _find_launcher_exe(self, launcher_dir: str) -> Optional[str]:
        ...
        for item in os.listdir(launcher_dir):
            if AppConfig.LAUNCHER_PROCESS_NAME in item and item.endswith('.exe'):
    ...
    def _backup_mods(self, game_dir: str) -> bool:
        ...
        for item in mod_files:
            ...
            if self._copy_item(src, dst):
                ...
                self.log_t('backup_failed', item, level="ERROR")
                return False
        return True
    ...
    def _wait_for_manual_click_and_capture(self) -> Optional[str]:
        """
        ...
        if not game_proc:
            raise RuntimeError(self.lang.t_format('timeout_no_game', AppConfig.GAME_START_TIMEOUT))
    ...
    def _launch_game_with_params(self, params: Optional[str]) -> bool:
        """
        使用捕获的参数启动游戏。
        ...
        try:
            cmd_args = [game_exe]
            if params:
                cmd_args.extend(shlex.split(params))
            subprocess.Popen(
                cmd_args,
    ...
    def _start_launch_process(self) -> None:
        ...
        worker_thread = threading.Thread(target=self._run_launch_process, daemon=False)
        worker_thread.start()
    ...
    def _run_launch_process(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if not launcher_exe:
                    raise RuntimeError(self.lang.t_format('launcher_not_found_in_dir', launcher_dir))
        ...
        except (OSError, subprocess.SubprocessError, ValueError, RuntimeError, FileNotFoundError) as e:
            ...
            try:
                ...
                self.logger.warning(f"Failed to restore mods during cleanup: {cleanup_error}")
            error_title = self.lang.t('error_title')
            error_msg = self.lang.t_format('process_error', str(e))
            self._set_button_state('normal')
            self.root.after(0, lambda: messagebox.showerror(error_title, error_msg))
